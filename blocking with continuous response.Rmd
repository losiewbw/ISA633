...
Now let's simulate some data for an analysis. By the way, this is something I do frequently to check things are set up correctly!
...
-   Randomized Complete Block Design
    ...
    -   Replicates are randomized within each block

-   Latin Squares Design

    -   2 blocking factors

    -   \# of treatments = \# of col blocks = \# of row blocks

    -   very efficient

-   Replicates or adding replicates reduces experimental error

    -   increases precision

## More than one Replicate Per Block

In the script experiment above each agent read each script a single time. In a randomized complete block design there is [exactly one replicate per block]{.underline}.
...
The model for this experiment is a little different. Because now there are *replicate experimental units* in each [block it is possible find the block by treatment interaction]{.underline}.

$$Y_{ij}=\mu+b_i+\tau_j+b\tau_{ij}+\epsilon_{ij}$$

-   We want to focus on the random error, which is the fourth term in the equation

But let's think about what we would like to conclude from this study. [We want to know which tee height is good for all golfers, not just for an individual golfer]{.underline}.

If the golfer by tee height interaction were significant in this model what would that imply?

-   Different heights are good for different golfers

Recall the [$MSE$]{.underline} estimates the variation of the response around the treatment mean.
...
-   individual golfer variation within a tee height

So if use the "[regular error term]{.underline}" we will not be able to generalize our conclusions to all golfers (from our random sample of golfers).
...
-   Use the golfer by tee height interaction as the error term
...
-   Df = 3 tee heights - 1 = 2

-   9 golfers -1 \*2 replicates = 16 residuals

Here is the **incorrect way**!!! -- DO NOT USE IN HOMEWORK

-   Tee.Hgt\*Golfer.ID = all possible interactions
