...
The solution to the problem of having a large run size and keep the same number of factors is to use just a piece of the full factorial experiment (a fraction).

9 main effects , 2 factor interactions, 3f,4f,5f,6f,7f,8f, 9 factor interactions.

if the coefficients are tiny, we can just ignore them in the model.
...
3.  **Hierarchical ordering principle** main effects are more likely to be important than two-factor interactions, and two-factor interactions are more likely to be important than three-factor interactions, and so forth.

4.  **Sequential experimentation**. It is possible to combine the runs of two (or more) fractional factorials to construct sequentially a larger design to estimate the factor effects and interactions of interest. - imp.
...
for the D column - multiply A,B,C columns to get +1 or -1

thats how we can create the half fraction.

D = the highest order of interaction
...
So what if we found A (main effect) to be active?

-   almost always assume 3 way interactions are negligible.

So what if we found A:B (2 factor interaction) to be active?
...
-   go back and look at the alias pattern
...
This is wrong, but here is what would happen if I included more effects than I have degrees of freedom to estimate:
...
```{r, echo=FALSE}
df<-read.csv("marketing experiment.csv")
df$ABCD<-df$A*df$B*df$C*df$D
df.half<-df[which(df$ABCD==1),]
write.csv(df.half[,-6], "marketing experiment half.csv", row.names = FALSE)
df.half2<-df[which(df$ABCD==-1),]
write.csv(df.half2[,-6], "marketing experiment half 2.csv", row.names = FALSE)
```

-   B:C is NA meaning, no df available to estimate that effect.
...
```{r}
df<-read.csv("marketing experiment half.csv")
...
-   if you suspect 2 FI are important you would have to add runs to your experiment.
...
```{r}
df2<-read.csv("marketing experiment half 2.csv")
...
![](Capture3.jpg) This code in R will give you the design above.
...
For this set up, resolution III is the best we can do. Resolution : the length of the shortest "word" . Main effects = 2 factor interactions (the 1st one)

-   res 3 is risky, chances are you will have to do a follow up.
...
Res 4 is pretty good.

FrF2 package automatically gives the highest resolution possible.
...
Can we go smaller? How about a 1/16 fraction? What resolution is this design? 2\^(8-4) = 16, res 4.
...
What to do? Add runs to the experiment.
...
We know that the variance-covariance matrix of the regression coefficients in a linear model is $\sigma^2(X'X)^{-1}$. We can't control $\sigma^2$ (experimental error). But we can control properties of $(X'X)^{-1}$.

In this case we will find design points that maximize the determinant of $(X'X)^{-1}$. Which minimizes the variance of the regression coefficients. $(X'X)^{-1}$ is our model matrix.

The ideal structure for a least squares design is to have an D data frame that has no correlation between the columns. The goal is to make the correlation as small as possible.
