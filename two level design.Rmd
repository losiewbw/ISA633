...
-   3,3,2,2,3 = 3\**3\**2\**2\**3 = 108 (which is a lot of webpages)
...
-   32 = 2\**2\**2\**2\**2 - $2^k$ (2 = \# of levels, k = #no. of factors)
...
The high and low are spread out as a far as possible in order to accentuate the signal or difference in response between the two levels. - this dictates the effect size you are trying to detect.
...
where alpha is at its low level = $$\alpha_{-}= \bar{y}_{-...}-\bar{y}_{....}$$

where alpha is at its high level = $\alpha_{+}= \bar{y}_{+...}-\bar{y}_{....}$\
...
The slope $\beta_A$ is just one half the effect, $E_{A}$. Or just the difference in the high-low averages divided by two.
...
we can estimate linear (main) effects - A,B,C

interactions - AB, BC, AC (2factor)

ABC (3 factor)

### Interactions

definition : the effect (beta-hat i.e. slope) of A on Y depends on the level of B.

beta-hat = (X'X)\^-1 . X' Y - method of least squares - method of estimating the coefficients in regression models
...
2 to the 4th - 2 level factorial, 4 factors

To be orthogonal it means that the model matrix

Model matrix times 1 over n is the correlation matrix.
...
Numerous small vendors push carts and sell food to tourists in Washington, DC, especially near The Mall. At one end of The Mall is the Lincoln Memorial, at the other end is the Capital Building. A man owns several of these carts and deploys them at either end of the The Mall. He wants to maximize his profits and so conducts an experiment. The levels of the factors are given below.
...
-   What kind of design is this -2\^4 factorial
...
The next step is to fit the full regression model (all possible main effects and interactions). Note, even though A, B, C, and D are technically factors we need to keep them coded as numeric in r. This will give us the correct effect estimates we are after (see above).
...
this is replicated 7500 times

original data had 7500\*16 rows.
